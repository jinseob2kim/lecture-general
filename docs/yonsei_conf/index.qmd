...
-   Event rate(proportion)의 **differecne** 이용
-   Non-inferiority margin도 **difference** scale
-   Racing, LOADSTAR, FLAVOUR

[Survival](https://nshi.jp/en/js/twosurvyrni/)

-   Event rate(hazard rate)의 **Ratio(HR)** 이용
-   Margin도 **HR**, difference scale로 바꿀수는 있음
-   Accural/FU time 고려 가능
-   RENOVATE-COMPLEX-PCI


## **샘플수방법 = 논문분석방법**

샘플수를 difference로 계산했다면, 실제분석도 difference로 (No HR)

- **RACING**: Proportion(rate 아님) difference + HR
- **LOADSTAR** : rate differecnce만, No HR
- **FLAVOUR**: Difference(Table), HR(본문, supplement) 같이 제시


샘플수를 HR로 계산했다면, 실제분석도 HR로 

- **RENOVATE-COMPLEX-PCI**: HR
...
## 번외: Landmark analysis

```{r}
fit3 <- survfit(Surv(time, status) ~ sex, data = colon)
jskm(fit3, mark = F, surv.scale = "percent", table = T, pval = T, pval.coord = c(350, 0.5), cut.landmark = 730, cumhaz = T)
```
...
<center><img src="img/nejmpp.png" width="50%"/></a></center>
