...
-   **네이티브 설치**: Git Bash만 있으면 됨 (WSL 불필요)
-   **WSL 설치**: Linux 환경 선호 시 대안
...
|---------------|----------------------|----------------------|
| **난이도**    | 쉬움                 | 보통                 |
| **필요한 것** | Git for Windows      | WSL2 + Ubuntu        |
| **R 연동**    | 기존 Windows R 사용  | WSL 안에 R 별도 설치 |
| **소요시간**  | 5분                  | 15-20분              |
| **장점**      | 간편, 기존 환경 유지 | Linux 환경, 샌드박싱 |

<br>

::: callout-tip
**대부분의 경우 네이티브 설치를 추천합니다**

이미 Windows에 R이 설치되어 있으면 그대로 사용 가능
:::
...
## 네이티브: Step 2 - Claude Code 설치

**PowerShell** 열고 한 줄 실행:
...
-   Linux 기반 R 패키지 사용 시 (시스템 라이브러리 의존성)
-   보안 샌드박싱이 필요한 경우
-   Linux 명령어에 익숙한 경우
...
```         
1. WSL2 설치          ← PowerShell에서 한 줄
    ↓
2. Node.js 설치       ← WSL Ubuntu에서
    ↓
3. Claude Code 설치   ← npm으로 설치
    ↓
4. R 설치             ← WSL Ubuntu에서
```
...
-   Ubuntu가 기본으로 설치됨
-   **재부팅 필요**
-   재부팅 후 Ubuntu 앱이 자동 실행 → 사용자명/비밀번호 설정

<br>

::: callout-note
**확인**: `wsl --version` 으로 WSL2인지 확인

WSL1이면: `wsl --set-default-version 2`

`wsl --install` 실패 시: BIOS에서 가상화(VT-x/AMD-V) 활성화 필요
:::

## WSL: Step 2 & 3 - Node.js + Claude Code
...
**WSL과 Windows 간 파일 접근**

| WSL 경로                       | Windows 경로               |
|--------------------------------|----------------------------|
| `/mnt/c/Users/본인/Documents/` | `C:\Users\본인\Documents\` |
| `/mnt/d/data/`                 | `D:\data\`                 |

<br>

``` bash
# WSL에서 Windows 문서 폴더로 이동
cd /mnt/c/Users/본인이름/Documents/my_research
claude
```

<br>

-   Claude Code가 WSL에서 R 코드 작성 + 실행
-   결과물(pptx, xlsx)은 `/mnt/c/...`에 저장 → Windows에서 바로 열기

::: callout-note
**한글 설정**: `sudo apt-get install -y language-pack-ko && echo 'export LANG=ko_KR.UTF-8' >> ~/.bashrc`
:::

## 폴더 구조 세팅

**네이티브든 WSL이든 동일한 프로젝트 구조**
...
| 상황 | 네이티브 | WSL |
|----|----|----|
| claude 명령어 안 됨 | PATH에 `~/.local/bin` 추가 | `npm` 재설치 |
| R 못 찾음 | Rscript PATH 확인 | `sudo apt install r-base` |
| 한글 깨짐 | Windows 설정에서 UTF-8 활성화 | `LANG=ko_KR.UTF-8` |
| 패키지 설치 에러 | Rtools 설치 | `apt-get` 시스템 라이브러리 |
