...
이에 맞춰 정부에서는 다양한 임상, 유전체 등 바이오 빅데이터를 구축 및 표준화하고 이를 이용한 분석과 서비스 개발을 촉진하겠다고 선언했습니다.
...
분석서비스를 위해 먼저 임상시험에 쓰이는 대부분의 분석들을 모듈화 하여 오픈소스 통계패키지로 개발할 것이며, 이 분석모듈들을 활용하여 맞춤형/범용 분석웹을 SaaS형태로 제공하겠습니다.
다음으로는, 분석결과를 입력으로 받아 보고서와 논문 초안으로 만들어주는 생성형AI를 개발할 것이며, 국가 R&D 를 수행하며 축적한 생성형AI 노하우를 활용하겠습니다.
...
최근 20년간 의학계열로 인재가 집중되었기 때문에 반드시 이쪽 분야에서 글로벌 인재와 기업이 나와야 하고, 이 인재들이 활약할 수 있도록 연구지원전문가라는 새로운 전문직을 만들겠습니다
...
IITP/NIPA 지원으로 개발한 저희 statgarten 프로젝트는 의학연구를 위한 오픈소스분석SW와 이 분석모듈들을 조립해 쉽게 맞춤형, 범용 분석웹을 만드는 기술이고, 범용웹은 무료 통계서비스 openstat을 글로벌 런칭했습니다.
...
저희 패키지를 일본에서 많이 인정해줘서, 활발히 이용되고 블로그에 리뷰되었습니다.
최근에서는 중국 의사와 개발자들의 issue, pull request 등 기여가 늘어나고 있습니다.
피드백과 업데이트 내용들은 자체 개발한 대시보드를 통해 실시간으로 파악하고 있습니다.

저희 패키지의 가장 큰 장점은 잘 정제된 공공빅데이터부터 교수님이 직접 만든 엑셀파일까지 온갖 종류의 그야말로 날 것의 데이터들을 다루면서 지속적으로 업데이트를 수행했다는 점과, 의학연구의 고급통계까지 커버한다는 점입니다. 의학연구에서 주로 쓰이는 R로 만들어졌기 때문에 최신 고급 통계방법론들을 빠르게 저희 프로젝트에 적용할 수 있습니다. 또한 각 분석기술들을 모듈화 했기 때문에 모듈들의 조합만으로 고객이 요청하는 분석웹을 바로 만들 수 있고, 고객은 웹에서 코딩없이 클릭만으로 다양한 옵션을 직접 적용할 수 있어 불필요한 커뮤니케이션을 50%이상 줄일 수 있습니다. 기존 통계프로그램은 분석 수행후 그 결과를 보고서나 논문용 테이블/그림으로 정리해야 했는데, 저희 웹에서는 바로 보고서용 테이블/그림 다운로드 기능을 제공하는 것도 장점입니다. 

지금까지 저희 기술들은 의학연구 즉 논문에 특화되어 있었는데, 이것을 신약 임상시험용으로 업그레이드하기 위해 글로벌 오픈소스 임상시험 프로젝트인 pharmaverse를 주목했습니다.
이 프로젝트는 글로벌 제약사 GSK/Roche/Atorus가 함께 개발하는 오픈소스 프로젝트로, 미국 FDA 승인을 위한 임상시험 데이터 입력부터 보고서 작성까지의 전 과정을 포함합니다. 특히 CDISC 표준데이터를 지원하는데 장점이 있인데요, 글로벌 제약사들이 참여하는만큼 다양한 임상시험 데이터를 다룰 수 있는 점이 장점이지만, 고급통계분석이 부족합니다.
...
작년부터는 100배 시장인 임상시험 분야에 진출했으며, 특히 다양하고 가공되지 않은 날것의 의료데이터를 다룬 노하우를 바탕으로 신라젠, 오츠카제약, 시선바이오머티리얼스, 씨젠, 도터, 에버엑스 등의 회사의 분석을 지원 중이며, 특히 최근 정형외과 재활 디지털치료기기 스타트업인 에버엑스의 확증임상시험 식약처승인을 지원하였습니다.
지금까지 투자나 대출없이 운영하였으며 최근 2년간 연평균 100% 매출성장을 이루었습니다.
아까 말씀드렸듯이 바이오산업에 대한민국의 50년 미래가 달려있다고 생각하며 인재육성에 힘쓰고 있습니다.
...
저희 개발팀은 최고의 통계SW 개발자로 각자 오픈소스 SW를 개발하여 공개했으며, 2건의 국제 컨퍼런스의 발표자로 선정되어 발표하였습니다.
...
저희의 제안을 본격적으로 말씀드리겠습니다. 기보유한 의학연구용 분석SW를 신약 임상시험용으로 업그레이드하고 상용화하여 글로벌 진출하는 것으로, 기술적으로는 분석서비스와 생성형AI 활용한 보고서/논문 생성 기술로 나눌 수 있습니다.
먼저 임상시험에 분석들을 모듈화 하여 오픈소스 통계패키지로 개발하고, 이 분석모듈들을 활용하여 맞춤형 분석웹을 제공하는 웹 개발 기술과, 흔히 쓰이는 분석기능들을 5가지로 분류한 범용 분석서비스를 만들어 SaaS형태로 제공할 것입니다.
...
아까 말씀드렸듯이 저희 분석패키지가 일본에서 많이 이용됩니다. 이에 맞춰 지난달 일본 오픈소스 밋업에서 저희 오픈소스 패키지를 소개했고, 매달 밋업에 참석할 예정입니다.
일본은 다케다제약을 포함 대표적인 바이오 강국이지만 디지털화가 느립니다. 그러나 유료SW이용에 대해서는 굉장히 열린 나라라서 첫 해외진출목표로 설정했습니다.
...
사실 저희는 5년전 Autoscaling 이 적용된 MSA를 구축한 적이 있었는데요, 당시에는 이용자가 거의 없어 클라우드가 부담이 되었고, 다시 물리서버로 옮겼습니다.
...
저번 달에 저희 개발리더가 글로벌 오픈소스 컨퍼런스의 바이오세션 발표자로 참여하였으며, 글로벌 제약사에서 많은 관심을 표현하셨습니다.
...
하나씩 말씀드리자면 먼저 매출은 작년 같은 기간 대비 50% 증가한 5억원을 목표로 잡았습니다. 임상시험 분석모듈과 분석웹 갯수는 공인인증기관의 인증을 받겠습니다.
보고서는 수요기관인 제약사나 병원연구자 통해 분석결과와 생성된 보고서를 보여드리고 평가를 받겠습니다.
